...
| Source file | [`src/PepperDash.Core/Logging/CrestronEnricher.cs`](../../../src/PepperDash.Core/Logging/CrestronEnricher.cs) |
| Language | C# |
| Declaration | `class CrestronEnricher` with declared base/contract list `ILogEventEnricher` |
| Accessibility | `public` |
| Namespace/module | `PepperDash.Core.Logging` |

## What

`CrestronEnricher` is a source-level implementation type with responsibilities defined by its members and inheritance. This description is grounded in its source declaration and declared inheritance rather than inferred product behavior.
...
Preserve the declared inheritance/contract relationship: `ILogEventEnricher`. Public methods declared in this source file include: `Enrich`. Use repository search to identify callers, implementers, serializers, tests, and configuration references before changing a public name or shape.
