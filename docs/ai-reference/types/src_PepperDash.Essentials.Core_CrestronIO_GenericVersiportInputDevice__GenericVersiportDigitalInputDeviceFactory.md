# GenericVersiportDigitalInputDeviceFactory
...
| Language | C# |
| Declaration | `class GenericVersiportDigitalInputDeviceFactory` with declared base/contract list `EssentialsDeviceFactory<GenericVersiportDigitalInputDevice>` |
| Accessibility | `public` |
| Namespace/module | `PepperDash.Essentials.Core.CrestronIO` |

## What

`GenericVersiportDigitalInputDeviceFactory` is a construction boundary that maps configuration or discovery inputs into concrete objects. This description is grounded in its source declaration and declared inheritance rather than inferred product behavior.
