# SetTopBoxThreePanelPageManager
...
| Language | C# |
| Declaration | `class SetTopBoxThreePanelPageManager` with declared base/contract list `ThreePanelPlusOnePageManager` |
| Accessibility | `public` |
| Namespace/module | `PepperDash.Essentials.Core.PageManagers` |

## What

`SetTopBoxThreePanelPageManager` is a source-level implementation type with responsibilities defined by its members and inheritance. This description is grounded in its source declaration and declared inheritance rather than inferred product behavior.
...
Preserve the declared inheritance/contract relationship: `ThreePanelPlusOnePageManager`. Public methods declared in this source file include: `Show`, `Hide`. Use repository search to identify callers, implementers, serializers, tests, and configuration references before changing a public name or shape.
