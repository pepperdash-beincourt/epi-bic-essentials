...
| Source file | [`src/PepperDash.Essentials.Core/Devices/EssentialsDeviceFactory.cs`](../../../src/PepperDash.Essentials.Core/Devices/EssentialsDeviceFactory.cs) |
| Language | C# |
| Declaration | `class EssentialsDeviceFactory<T>` with declared base/contract list `IDeviceFactory where T:EssentialsDevice` |
| Accessibility | `public` |
| Namespace/module | `PepperDash.Essentials.Core` |

## What

`EssentialsDeviceFactory<T>` is a construction boundary that maps configuration or discovery inputs into concrete objects. This description is grounded in its source declaration and declared inheritance rather than inferred product behavior.
...
Preserve the declared inheritance/contract relationship: `IDeviceFactory where T:EssentialsDevice`. Public methods declared in this source file include: `BuildDevice`. Use repository search to identify callers, implementers, serializers, tests, and configuration references before changing a public name or shape.
