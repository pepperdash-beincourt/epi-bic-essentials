...
| Language | C# |
| Declaration | `class RetriggerableTimerFactory` with declared base/contract list `EssentialsDeviceFactory<RetriggerableTimer>` |
| Accessibility | `public` |
| Namespace/module | `PepperDash.Essentials.Core.Timers` |

## What

`RetriggerableTimerFactory` is a construction boundary that maps configuration or discovery inputs into concrete objects. This description is grounded in its source declaration and declared inheritance rather than inferred product behavior.
...
Preserve the declared inheritance/contract relationship: `EssentialsDeviceFactory<RetriggerableTimer>`. Public methods declared in this source file include: `StartTimer`, `StopTimer`, `BuildDevice`. Use repository search to identify callers, implementers, serializers, tests, and configuration references before changing a public name or shape.
