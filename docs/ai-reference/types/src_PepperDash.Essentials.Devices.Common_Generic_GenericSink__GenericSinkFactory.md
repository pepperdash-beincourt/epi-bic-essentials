...
| Language | C# |
| Declaration | `class GenericSinkFactory` with declared base/contract list `EssentialsDeviceFactory<GenericSink>` |
| Accessibility | `public` |
| Namespace/module | `PepperDash.Essentials.Devices.Common.Generic` |

## What

`GenericSinkFactory` is a construction boundary that maps configuration or discovery inputs into concrete objects. This description is grounded in its source declaration and declared inheritance rather than inferred product behavior.
...
Before proposing a change, read this declaration, its full source file, all repository references to `GenericSinkFactory`, and its test coverage. Do not invent configuration keys, payload fields, interface members, or lifecycle ordering. Report the affected source files, tests, and consumer boundaries with any proposed change.
