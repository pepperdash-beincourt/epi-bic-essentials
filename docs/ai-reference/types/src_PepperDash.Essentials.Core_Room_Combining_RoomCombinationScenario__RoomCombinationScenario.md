...
| Source file | [`src/PepperDash.Essentials.Core/Room/Combining/RoomCombinationScenario.cs`](../../../src/PepperDash.Essentials.Core/Room/Combining/RoomCombinationScenario.cs) |
| Language | C# |
| Declaration | `class RoomCombinationScenario` with declared base/contract list `IRoomCombinationScenario, IKeyName` |
| Accessibility | `public` |
| Namespace/module | `PepperDash.Essentials.Core` |

## What

`RoomCombinationScenario` is a source-level implementation type with responsibilities defined by its members and inheritance. This description is grounded in its source declaration and declared inheritance rather than inferred product behavior.
...
Preserve the declared inheritance/contract relationship: `IRoomCombinationScenario, IKeyName`. Public methods declared in this source file include: `Activate`, `Deactivate`. Use repository search to identify callers, implementers, serializers, tests, and configuration references before changing a public name or shape.
...
Before proposing a change, read this declaration, its full source file, all repository references to `RoomCombinationScenario`, and its test coverage. Do not invent configuration keys, payload fields, interface members, or lifecycle ordering. Report the affected source files, tests, and consumer boundaries with any proposed change.
