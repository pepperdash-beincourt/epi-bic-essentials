...
| Language | C# |
| Declaration | `interface IHasPowerControlWithFeedback` with declared base/contract list `IHasPowerControl` |
| Accessibility | `public` |
| Namespace/module | `PepperDash.Essentials.Core` |

## What

`IHasPowerControlWithFeedback` is a contract that callers and implementations use to agree on behavior. This description is grounded in its source declaration and declared inheritance rather than inferred product behavior.
...
Preserve the declared inheritance/contract relationship: `IHasPowerControl`. Public methods declared in this source file include: `LinkButtons`, `UnlinkButtons`. Use repository search to identify callers, implementers, serializers, tests, and configuration references before changing a public name or shape.
