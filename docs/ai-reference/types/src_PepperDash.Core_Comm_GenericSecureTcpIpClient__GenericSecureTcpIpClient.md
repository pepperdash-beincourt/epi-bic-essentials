...
| Source file | [`src/PepperDash.Core/Comm/GenericSecureTcpIpClient.cs`](../../../src/PepperDash.Core/Comm/GenericSecureTcpIpClient.cs) |
| Language | C# |
| Declaration | `class GenericSecureTcpIpClient` with declared base/contract list `Device, ISocketStatusWithStreamDebugging, IAutoReconnect` |
| Accessibility | `public` |
| Namespace/module | `PepperDash.Core` |

## What

`GenericSecureTcpIpClient` is a device-model type that owns lifecycle, controls, feedback, or communication responsibilities. This description is grounded in its source declaration and declared inheritance rather than inferred product behavior.
...
Before proposing a change, read this declaration, its full source file, all repository references to `GenericSecureTcpIpClient`, and its test coverage. Do not invent configuration keys, payload fields, interface members, or lifecycle ordering. Report the affected source files, tests, and consumer boundaries with any proposed change.
