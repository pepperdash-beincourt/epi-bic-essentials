...
| Source file | [`src/PepperDash.Essentials.MobileControl/WebSocketServer/MobileControlWebsocketServer.cs`](../../../src/PepperDash.Essentials.MobileControl/WebSocketServer/MobileControlWebsocketServer.cs) |
| Language | C# |
| Declaration | `class MobileControlWebsocketServer` with declared base/contract list `EssentialsDevice` |
| Accessibility | `public` |
| Namespace/module | `PepperDash.Essentials.WebSocketServer` |

## What

`MobileControlWebsocketServer` is a device-model type that owns lifecycle, controls, feedback, or communication responsibilities. This description is grounded in its source declaration and declared inheritance rather than inferred product behavior.
...
Preserve the declared inheritance/contract relationship: `EssentialsDevice`. Public methods declared in this source file include: `SetWebsocketLogLevel`, `UpdateSecret`, `RegisterUiClient`, `UpdateClientId`, `RegisterLegacyUiClient`, `PrintSessionData`, `StopServer`, `SendMessageToAllClients`, `SendMessageToClient`. Use repository search to identify callers, implementers, serializers, tests, and configuration references before changing a public name or shape.
