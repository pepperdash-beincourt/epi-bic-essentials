...
| Language | C# |
| Declaration | `class ActionSequence` with declared base/contract list `EssentialsDevice` |
| Accessibility | `public` |
| Namespace/module | `PepperDash.Essentials.Core.Utilities` |

## What

`ActionSequence` is a device-model type that owns lifecycle, controls, feedback, or communication responsibilities. This description is grounded in its source declaration and declared inheritance rather than inferred product behavior.
...
Before proposing a change, read this declaration, its full source file, all repository references to `ActionSequence`, and its test coverage. Do not invent configuration keys, payload fields, interface members, or lifecycle ordering. Report the affected source files, tests, and consumer boundaries with any proposed change.
