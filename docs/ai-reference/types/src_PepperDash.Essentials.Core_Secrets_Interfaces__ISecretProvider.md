...
| Source file | [`src/PepperDash.Essentials.Core/Secrets/Interfaces.cs`](../../../src/PepperDash.Essentials.Core/Secrets/Interfaces.cs) |
| Language | C# |
| Declaration | `interface ISecretProvider` with declared base/contract list `IKeyed` |
| Accessibility | `public` |
| Namespace/module | `PepperDash.Essentials.Core` |

## What

`ISecretProvider` is a contract that callers and implementations use to agree on behavior. This description is grounded in its source declaration and declared inheritance rather than inferred product behavior.
