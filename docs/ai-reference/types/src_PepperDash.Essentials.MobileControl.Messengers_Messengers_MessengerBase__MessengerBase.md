...
| Source file | [`src/PepperDash.Essentials.MobileControl.Messengers/Messengers/MessengerBase.cs`](../../../src/PepperDash.Essentials.MobileControl.Messengers/Messengers/MessengerBase.cs) |
| Language | C# |
| Declaration | `class MessengerBase` with declared base/contract list `EssentialsDevice, IMobileControlMessenger` |
| Accessibility | `public` |
| Namespace/module | `PepperDash.Essentials.AppServer.Messengers` |

## What

`MessengerBase` is a message-handling boundary that translates between transport payloads and application behavior. This description is grounded in its source declaration and declared inheritance rather than inferred product behavior.
...
Preserve the declared inheritance/contract relationship: `EssentialsDevice, IMobileControlMessenger`. Public methods declared in this source file include: `RegisterWithAppServer`, `GetActionPaths`, `UnsubscribeClient`. Use repository search to identify callers, implementers, serializers, tests, and configuration references before changing a public name or shape.
