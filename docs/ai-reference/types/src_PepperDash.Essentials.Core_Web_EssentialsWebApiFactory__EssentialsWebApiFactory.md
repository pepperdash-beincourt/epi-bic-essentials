...
| Source file | [`src/PepperDash.Essentials.Core/Web/EssentialsWebApiFactory.cs`](../../../src/PepperDash.Essentials.Core/Web/EssentialsWebApiFactory.cs) |
| Language | C# |
| Declaration | `class EssentialsWebApiFactory` with declared base/contract list `EssentialsDeviceFactory<EssentialsWebApi>` |
| Accessibility | `public` |
| Namespace/module | `PepperDash.Essentials.Core.Web` |

## What

`EssentialsWebApiFactory` is a construction boundary that maps configuration or discovery inputs into concrete objects. This description is grounded in its source declaration and declared inheritance rather than inferred product behavior.
...
Before proposing a change, read this declaration, its full source file, all repository references to `EssentialsWebApiFactory`, and its test coverage. Do not invent configuration keys, payload fields, interface members, or lifecycle ordering. Report the affected source files, tests, and consumer boundaries with any proposed change.
