...
| Source file | [`src/PepperDash.Essentials.Core/Config/BasicConfig.cs`](../../../src/PepperDash.Essentials.Core/Config/BasicConfig.cs) |
| Language | C# |
| Declaration | `class BasicConfig` |
| Accessibility | `public` |
| Namespace/module | `PepperDash.Essentials.Core.Config` |

## What

`BasicConfig` is a configuration or data-transfer shape used to deserialize and validate structured input. This description is grounded in its source declaration and declared inheritance rather than inferred product behavior.
...
Public methods declared in this source file include: `GetSourceListForKey`, `GetDestinationListForKey`, `GetAudioControlPointListForKey`, `GetCameraListForKey`, `GetDeviceForKey`. Use repository search to identify callers, implementers, serializers, tests, and configuration references before changing a public name or shape.
...
Before proposing a change, read this declaration, its full source file, all repository references to `BasicConfig`, and its test coverage. Do not invent configuration keys, payload fields, interface members, or lifecycle ordering. Report the affected source files, tests, and consumer boundaries with any proposed change.
