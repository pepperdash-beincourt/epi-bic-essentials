...
| Source file | [`src/PepperDash.Core/Comm/GenericSshClient.cs`](../../../src/PepperDash.Core/Comm/GenericSshClient.cs) |
| Language | C# |
| Declaration | `class SshConnectionChangeEventArgs` with declared base/contract list `EventArgs` |
| Accessibility | `public` |
| Namespace/module | `PepperDash.Core` |

## What

`SshConnectionChangeEventArgs` is an event payload type that carries state-change information between collaborators. This description is grounded in its source declaration and declared inheritance rather than inferred product behavior.
...
Preserve the declared inheritance/contract relationship: `EventArgs`. Public methods declared in this source file include: `Connect`, `Disconnect`, `SendText`, `SendBytes`. Use repository search to identify callers, implementers, serializers, tests, and configuration references before changing a public name or shape.
