...
| Source file | [`src/PepperDash.Essentials.Core/Touchpanels/TriListExtensions.cs`](../../../src/PepperDash.Essentials.Core/Touchpanels/TriListExtensions.cs) |
| Language | C# |
| Declaration | `class SigAndTriListExtensions` |
| Accessibility | `public` |
| Namespace/module | `PepperDash.Essentials.Core` |

## What

`SigAndTriListExtensions` is a source-level implementation type with responsibilities defined by its members and inheritance. This description is grounded in its source declaration and declared inheritance rather than inferred product behavior.
...
Public methods declared in this source file include: `SetBoolSigAction`, `SetSigTrueAction`, `SetSigFalseAction`, `SetSigHeldAction`, `SetUShortSigAction`, `SetStringSigAction`, `ClearSigAction`, `ClearBoolSigAction`, `ClearUShortSigAction`, `ClearStringSigAction`, `ClearAllSigActions`, `SetBool`. Use repository search to identify callers, implementers, serializers, tests, and configuration references before changing a public name or shape.
