...
| Source file | [`src/PepperDash.Essentials.MobileControl/MobileControlFactory.cs`](../../../src/PepperDash.Essentials.MobileControl/MobileControlFactory.cs) |
| Language | C# |
| Declaration | `class MobileControlFactory` |
| Accessibility | `public` |
| Namespace/module | `PepperDash.Essentials` |

## What

`MobileControlFactory` is a construction boundary that maps configuration or discovery inputs into concrete objects. This description is grounded in its source declaration and declared inheritance rather than inferred product behavior.
...
Before proposing a change, read this declaration, its full source file, all repository references to `MobileControlFactory`, and its test coverage. Do not invent configuration keys, payload fields, interface members, or lifecycle ordering. Report the affected source files, tests, and consumer boundaries with any proposed change.
