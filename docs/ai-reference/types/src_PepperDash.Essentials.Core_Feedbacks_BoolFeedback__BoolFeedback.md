...
| Source file | [`src/PepperDash.Essentials.Core/Feedbacks/BoolFeedback.cs`](../../../src/PepperDash.Essentials.Core/Feedbacks/BoolFeedback.cs) |
| Language | C# |
| Declaration | `class BoolFeedback` with declared base/contract list `Feedback` |
| Accessibility | `public` |
| Namespace/module | `PepperDash.Essentials.Core` |

## What

`BoolFeedback` is a source-level implementation type with responsibilities defined by its members and inheritance. This description is grounded in its source declaration and declared inheritance rather than inferred product behavior.
...
Preserve the declared inheritance/contract relationship: `Feedback`. Public methods declared in this source file include: `SetValueFunc`, `FireUpdate`, `LinkInputSig`, `UnlinkInputSig`, `LinkComplementInputSig`, `UnlinkComplementInputSig`, `LinkCrestronFeedback`, `UnlinkCrestronFeedback`, `ToString`, `SetTestValue`. Use repository search to identify callers, implementers, serializers, tests, and configuration references before changing a public name or shape.
...
Before proposing a change, read this declaration, its full source file, all repository references to `BoolFeedback`, and its test coverage. Do not invent configuration keys, payload fields, interface members, or lifecycle ordering. Report the affected source files, tests, and consumer boundaries with any proposed change.
