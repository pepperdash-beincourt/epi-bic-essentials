...
| Source file | [`src/PepperDash.Essentials.Core/Comm and IR/GenericComm.cs`](../../../src/PepperDash.Essentials.Core/Comm and IR/GenericComm.cs) |
| Language | C# |
| Declaration | `class GenericCommFactory` with declared base/contract list `EssentialsDeviceFactory<GenericComm>` |
| Accessibility | `public` |
| Namespace/module | `PepperDash.Essentials.Core` |

## What

`GenericCommFactory` is a construction boundary that maps configuration or discovery inputs into concrete objects. This description is grounded in its source declaration and declared inheritance rather than inferred product behavior.
...
Preserve the declared inheritance/contract relationship: `EssentialsDeviceFactory<GenericComm>`. Public methods declared in this source file include: `BuildDevice`, `SetPortConfig`, `LinkToApi`. Use repository search to identify callers, implementers, serializers, tests, and configuration references before changing a public name or shape.
...
Before proposing a change, read this declaration, its full source file, all repository references to `GenericCommFactory`, and its test coverage. Do not invent configuration keys, payload fields, interface members, or lifecycle ordering. Report the affected source files, tests, and consumer boundaries with any proposed change.
