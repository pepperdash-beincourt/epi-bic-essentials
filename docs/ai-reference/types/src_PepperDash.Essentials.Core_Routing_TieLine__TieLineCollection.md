...
| Language | C# |
| Declaration | `class TieLineCollection` with declared base/contract list `List<TieLine>` |
| Accessibility | `public` |
| Namespace/module | `PepperDash.Essentials.Core` |

## What

`TieLineCollection` is a source-level implementation type with responsibilities defined by its members and inheritance. This description is grounded in its source declaration and declared inheritance rather than inferred product behavior.
...
Preserve the declared inheritance/contract relationship: `List<TieLine>`. Public methods declared in this source file include: `Activate`, `Deactivate`, `ToString`. Use repository search to identify callers, implementers, serializers, tests, and configuration references before changing a public name or shape.
