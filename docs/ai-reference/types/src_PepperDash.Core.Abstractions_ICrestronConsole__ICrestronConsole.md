...
| Source file | [`src/PepperDash.Core.Abstractions/ICrestronConsole.cs`](../../../src/PepperDash.Core.Abstractions/ICrestronConsole.cs) |
| Language | C# |
| Declaration | `interface ICrestronConsole` |
| Accessibility | `public` |
| Namespace/module | `PepperDash.Core.Abstractions` |

## What

`ICrestronConsole` is a contract that callers and implementations use to agree on behavior. This description is grounded in its source declaration and declared inheritance rather than inferred product behavior.
...
Before proposing a change, read this declaration, its full source file, all repository references to `ICrestronConsole`, and its test coverage. Do not invent configuration keys, payload fields, interface members, or lifecycle ordering. Report the affected source files, tests, and consumer boundaries with any proposed change.
