...
| Source file | [`src/PepperDash.Essentials.MobileControl/MobileControlSystemController.cs`](../../../src/PepperDash.Essentials.MobileControl/MobileControlSystemController.cs) |
| Language | C# |
| Declaration | `class MobileControlSystemController` with declared base/contract list `EssentialsDevice, IMobileControl` |
| Accessibility | `public` |
| Namespace/module | `PepperDash.Essentials` |

## What

`MobileControlSystemController` is a device-model type that owns lifecycle, controls, feedback, or communication responsibilities. This description is grounded in its source declaration and declared inheritance rather than inferred product behavior.
...
Preserve the declared inheritance/contract relationship: `EssentialsDevice, IMobileControl`. Public methods declared in this source file include: `AddDefaultMessengersForDevice`, `CheckForDeviceMessenger`, `AddDeviceMessenger`, `GetAppServer`, `LinkSystemMonitorToAppServer`, `PrintActionDictionaryPaths`, `RemoveAction`, `GetRoomBridge`, `GetRoomMessenger`, `RegisterSystemToServer`, `GetConfigWithPluginVersion`, `SetClientUrl`. Use repository search to identify callers, implementers, serializers, tests, and configuration references before changing a public name or shape.
