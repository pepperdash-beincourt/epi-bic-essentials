...
| Source file | [`src/PepperDash.Core/SystemInfo/SystemInfoToSimpl.cs`](../../../src/PepperDash.Core/SystemInfo/SystemInfoToSimpl.cs) |
| Language | C# |
| Declaration | `class SystemInfoToSimpl` |
| Accessibility | `public` |
| Namespace/module | `PepperDash.Core.SystemInfo` |

## What

`SystemInfoToSimpl` is a structured representation of data exchanged between components. This description is grounded in its source declaration and declared inheritance rather than inferred product behavior.
...
Public methods declared in this source file include: `GetProcessorInfo`, `GetEthernetInfo`, `GetControlSubnetInfo`, `GetProgramInfoByIndex`, `RefreshProcessorUptime`, `RefreshProgramUptimeByIndex`, `SendConsoleCommand`. Use repository search to identify callers, implementers, serializers, tests, and configuration references before changing a public name or shape.
