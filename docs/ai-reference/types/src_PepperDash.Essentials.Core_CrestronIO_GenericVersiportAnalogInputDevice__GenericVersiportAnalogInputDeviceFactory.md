# GenericVersiportAnalogInputDeviceFactory
...
| Source file | [`src/PepperDash.Essentials.Core/CrestronIO/GenericVersiportAnalogInputDevice.cs`](../../../src/PepperDash.Essentials.Core/CrestronIO/GenericVersiportAnalogInputDevice.cs) |
| Language | C# |
| Declaration | `class GenericVersiportAnalogInputDeviceFactory` with declared base/contract list `EssentialsDeviceFactory<GenericVersiportAnalogInputDevice>` |
| Accessibility | `public` |
| Namespace/module | `PepperDash.Essentials.Core.CrestronIO` |

## What

`GenericVersiportAnalogInputDeviceFactory` is a construction boundary that maps configuration or discovery inputs into concrete objects. This description is grounded in its source declaration and declared inheritance rather than inferred product behavior.
...
Preserve the declared inheritance/contract relationship: `EssentialsDeviceFactory<GenericVersiportAnalogInputDevice>`. Public methods declared in this source file include: `SetMinimumChange`, `LinkToApi`, `GetVersiportDigitalInput`, `BuildDevice`. Use repository search to identify callers, implementers, serializers, tests, and configuration references before changing a public name or shape.
