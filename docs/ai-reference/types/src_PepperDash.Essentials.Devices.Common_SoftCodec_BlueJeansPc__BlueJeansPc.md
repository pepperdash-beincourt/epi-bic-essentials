...
| Source file | [`src/PepperDash.Essentials.Devices.Common/SoftCodec/BlueJeansPc.cs`](../../../src/PepperDash.Essentials.Devices.Common/SoftCodec/BlueJeansPc.cs) |
| Language | C# |
| Declaration | `class BlueJeansPc` with declared base/contract list `InRoomPc, IRunRouteAction, IRoutingSinkWithFeedback` |
| Accessibility | `public` |
| Namespace/module | `PepperDash.Essentials.Devices.Common.SoftCodec` |

## What

`BlueJeansPc` is a source-level implementation type with responsibilities defined by its members and inheritance. This description is grounded in its source declaration and declared inheritance rather than inferred product behavior.
...
Preserve the declared inheritance/contract relationship: `InRoomPc, IRunRouteAction, IRoutingSinkWithFeedback`. Public methods declared in this source file include: `ExecuteSwitch`, `SetCurrentSource`, `RunRouteAction`. Use repository search to identify callers, implementers, serializers, tests, and configuration references before changing a public name or shape.
