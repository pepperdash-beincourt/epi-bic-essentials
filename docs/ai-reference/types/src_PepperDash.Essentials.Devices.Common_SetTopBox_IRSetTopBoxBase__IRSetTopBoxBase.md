...
| Source file | [`src/PepperDash.Essentials.Devices.Common/SetTopBox/IRSetTopBoxBase.cs`](../../../src/PepperDash.Essentials.Devices.Common/SetTopBox/IRSetTopBoxBase.cs) |
| Language | C# |
| Declaration | `class IRSetTopBoxBase` with declared base/contract list `EssentialsBridgeableDevice, ISetTopBoxControls, IRoutingSource, IRoutingOutputs, IUsageTracking, IHasPowerControl, ITvPresetsProvider` |
| Accessibility | `public` |
| Namespace/module | `PepperDash.Essentials.Devices.Common` |

## What

`IRSetTopBoxBase` is a source-level implementation type with responsibilities defined by its members and inheritance. This description is grounded in its source declaration and declared inheritance rather than inferred product behavior.
...
Preserve the declared inheritance/contract relationship: `EssentialsBridgeableDevice, ISetTopBoxControls, IRoutingSource, IRoutingOutputs, IUsageTracking, IHasPowerControl, ITvPresetsProvider`. Public methods declared in this source file include: `LoadPresets`, `DvrList`, `Replay`, `Up`, `Down`, `Left`, `Right`, `Select`, `Menu`, `Exit`, `Digit0`, `Digit1`. Use repository search to identify callers, implementers, serializers, tests, and configuration references before changing a public name or shape.
