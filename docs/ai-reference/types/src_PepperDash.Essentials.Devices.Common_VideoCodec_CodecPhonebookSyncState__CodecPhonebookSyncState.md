...
| Source file | [`src/PepperDash.Essentials.Devices.Common/VideoCodec/CodecPhonebookSyncState.cs`](../../../src/PepperDash.Essentials.Devices.Common/VideoCodec/CodecPhonebookSyncState.cs) |
| Language | C# |
| Declaration | `class CodecPhonebookSyncState` with declared base/contract list `IKeyed` |
| Accessibility | `public` |
| Namespace/module | `PepperDash.Essentials.Devices.Common.VideoCodec` |

## What

`CodecPhonebookSyncState` is a source-level implementation type with responsibilities defined by its members and inheritance. This description is grounded in its source declaration and declared inheritance rather than inferred product behavior.
...
Preserve the declared inheritance/contract relationship: `IKeyed`. Public methods declared in this source file include: `InitialPhonebookFoldersReceived`, `PhonebookRootEntriesReceived`, `SetPhonebookHasFolders`, `SetNumberOfContacts`, `CodecDisconnected`. Use repository search to identify callers, implementers, serializers, tests, and configuration references before changing a public name or shape.
...
Before proposing a change, read this declaration, its full source file, all repository references to `CodecPhonebookSyncState`, and its test coverage. Do not invent configuration keys, payload fields, interface members, or lifecycle ordering. Report the affected source files, tests, and consumer boundaries with any proposed change.
