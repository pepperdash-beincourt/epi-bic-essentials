# GenericVersiportDigitalOutputDeviceFactory
...
| Source file | [`src/PepperDash.Essentials.Core/CrestronIO/GenericVersiportOutputDevice.cs`](../../../src/PepperDash.Essentials.Core/CrestronIO/GenericVersiportOutputDevice.cs) |
| Language | C# |
| Declaration | `class GenericVersiportDigitalOutputDeviceFactory` with declared base/contract list `EssentialsDeviceFactory<GenericVersiportDigitalInputDevice>` |
| Accessibility | `public` |
| Namespace/module | `PepperDash.Essentials.Core.CrestronIO` |

## What

`GenericVersiportDigitalOutputDeviceFactory` is a construction boundary that maps configuration or discovery inputs into concrete objects. This description is grounded in its source declaration and declared inheritance rather than inferred product behavior.
...
Preserve the declared inheritance/contract relationship: `EssentialsDeviceFactory<GenericVersiportDigitalInputDevice>`. Public methods declared in this source file include: `SetOutput`, `LinkToApi`, `GetVersiportDigitalOutput`, `BuildDevice`. Use repository search to identify callers, implementers, serializers, tests, and configuration references before changing a public name or shape.
