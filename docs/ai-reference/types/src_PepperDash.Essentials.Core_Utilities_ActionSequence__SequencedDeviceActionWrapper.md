...
| Language | C# |
| Declaration | `class SequencedDeviceActionWrapper` with declared base/contract list `DeviceActionWrapper` |
| Accessibility | `public` |
| Namespace/module | `PepperDash.Essentials.Core.Utilities` |

## What

`SequencedDeviceActionWrapper` is a device-model type that owns lifecycle, controls, feedback, or communication responsibilities. This description is grounded in its source declaration and declared inheritance rather than inferred product behavior.
