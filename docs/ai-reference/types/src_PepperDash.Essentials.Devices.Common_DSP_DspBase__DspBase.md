...
| Source file | [`src/PepperDash.Essentials.Devices.Common/DSP/DspBase.cs`](../../../src/PepperDash.Essentials.Devices.Common/DSP/DspBase.cs) |
| Language | C# |
| Declaration | `class DspBase` with declared base/contract list `EssentialsDevice, ILevelControls` |
| Accessibility | `public` |
| Namespace/module | `PepperDash.Essentials.Devices.Common.DSP` |

## What

`DspBase` is a device-model type that owns lifecycle, controls, feedback, or communication responsibilities. This description is grounded in its source declaration and declared inheritance rather than inferred product behavior.
...
Before proposing a change, read this declaration, its full source file, all repository references to `DspBase`, and its test coverage. Do not invent configuration keys, payload fields, interface members, or lifecycle ordering. Report the affected source files, tests, and consumer boundaries with any proposed change.
