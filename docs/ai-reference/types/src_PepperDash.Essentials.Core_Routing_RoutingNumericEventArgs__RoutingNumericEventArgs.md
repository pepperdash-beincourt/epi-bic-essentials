...
| Source file | [`src/PepperDash.Essentials.Core/Routing/RoutingNumericEventArgs.cs`](../../../src/PepperDash.Essentials.Core/Routing/RoutingNumericEventArgs.cs) |
| Language | C# |
| Declaration | `class RoutingNumericEventArgs` with declared base/contract list `EventArgs` |
| Accessibility | `public` |
| Namespace/module | `PepperDash.Essentials.Core` |

## What

`RoutingNumericEventArgs` is an event payload type that carries state-change information between collaborators. This description is grounded in its source declaration and declared inheritance rather than inferred product behavior.
