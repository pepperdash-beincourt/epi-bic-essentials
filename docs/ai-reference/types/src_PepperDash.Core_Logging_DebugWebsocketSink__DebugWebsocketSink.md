...
| Language | C# |
| Declaration | `class DebugWebsocketSink` with declared base/contract list `ILogEventSink, IKeyed` |
...
Preserve the declared inheritance/contract relationship: `ILogEventSink, IKeyed`. Public methods declared in this source file include: `Emit`, `StartServerAndSetPort`, `StopServer`, `DebugWebsocketSink`. Use repository search to identify callers, implementers, serializers, tests, and configuration references before changing a public name or shape.
