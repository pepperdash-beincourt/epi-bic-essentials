...
| Language | C# |
| Declaration | `class GenericSshClient` with declared base/contract list `Device, ISocketStatusWithStreamDebugging, IAutoReconnect` |
| Accessibility | `public` |
| Namespace/module | `PepperDash.Core` |

## What

`GenericSshClient` is a device-model type that owns lifecycle, controls, feedback, or communication responsibilities. This description is grounded in its source declaration and declared inheritance rather than inferred product behavior.
