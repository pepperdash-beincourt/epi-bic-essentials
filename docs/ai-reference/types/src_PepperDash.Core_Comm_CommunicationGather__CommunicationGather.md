...
| Source file | [`src/PepperDash.Core/Comm/CommunicationGather.cs`](../../../src/PepperDash.Core/Comm/CommunicationGather.cs) |
| Language | C# |
| Declaration | `class CommunicationGather` |
| Accessibility | `public` |
| Namespace/module | `PepperDash.Core` |

## What

`CommunicationGather` is a source-level implementation type with responsibilities defined by its members and inheritance. This description is grounded in its source declaration and declared inheritance rather than inferred product behavior.
...
Public methods declared in this source file include: `LineReceivedHandler`, `Stop`. Use repository search to identify callers, implementers, serializers, tests, and configuration references before changing a public name or shape.
