...
| Source file | [`src/PepperDash.Essentials.Core/Secrets/SecretsManager.cs`](../../../src/PepperDash.Essentials.Core/Secrets/SecretsManager.cs) |
| Language | C# |
| Declaration | `class SecretsManager` |
| Accessibility | `public` |
| Namespace/module | `PepperDash.Essentials.Core` |

## What

`SecretsManager` is a source-level implementation type with responsibilities defined by its members and inheritance. This description is grounded in its source declaration and declared inheritance rather than inferred product behavior.
...
Public methods declared in this source file include: `Initialize`, `GetSecretProviderByKey`, `GetProviderInfo`, `ListProviders`, `AddSecretProvider`. Use repository search to identify callers, implementers, serializers, tests, and configuration references before changing a public name or shape.
