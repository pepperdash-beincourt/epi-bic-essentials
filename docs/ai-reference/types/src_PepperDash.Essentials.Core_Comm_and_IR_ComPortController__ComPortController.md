...
| Source file | [`src/PepperDash.Essentials.Core/Comm and IR/ComPortController.cs`](../../../src/PepperDash.Essentials.Core/Comm and IR/ComPortController.cs) |
| Language | C# |
| Declaration | `class ComPortController` with declared base/contract list `Device, IBasicCommunicationWithStreamDebugging` |
| Accessibility | `public` |
| Namespace/module | `PepperDash.Essentials.Core` |

## What

`ComPortController` is a device-model type that owns lifecycle, controls, feedback, or communication responsibilities. This description is grounded in its source declaration and declared inheritance rather than inferred product behavior.
...
Preserve the declared inheritance/contract relationship: `Device, IBasicCommunicationWithStreamDebugging`. Public methods declared in this source file include: `Deactivate`, `SendText`, `SendBytes`, `Connect`, `Disconnect`, `SimulateReceive`. Use repository search to identify callers, implementers, serializers, tests, and configuration references before changing a public name or shape.
...
Before proposing a change, read this declaration, its full source file, all repository references to `ComPortController`, and its test coverage. Do not invent configuration keys, payload fields, interface members, or lifecycle ordering. Report the affected source files, tests, and consumer boundaries with any proposed change.
