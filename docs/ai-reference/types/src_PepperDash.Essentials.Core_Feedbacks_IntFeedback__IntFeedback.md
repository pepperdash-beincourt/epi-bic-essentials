...
| Source file | [`src/PepperDash.Essentials.Core/Feedbacks/IntFeedback.cs`](../../../src/PepperDash.Essentials.Core/Feedbacks/IntFeedback.cs) |
| Language | C# |
| Declaration | `class IntFeedback` with declared base/contract list `Feedback` |
| Accessibility | `public` |
| Namespace/module | `PepperDash.Essentials.Core` |

## What

`IntFeedback` is a source-level implementation type with responsibilities defined by its members and inheritance. This description is grounded in its source declaration and declared inheritance rather than inferred product behavior.
...
Before proposing a change, read this declaration, its full source file, all repository references to `IntFeedback`, and its test coverage. Do not invent configuration keys, payload fields, interface members, or lifecycle ordering. Report the affected source files, tests, and consumer boundaries with any proposed change.
