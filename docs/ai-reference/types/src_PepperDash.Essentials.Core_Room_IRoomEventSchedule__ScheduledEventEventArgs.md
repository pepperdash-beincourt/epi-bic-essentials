...
| Source file | [`src/PepperDash.Essentials.Core/Room/IRoomEventSchedule.cs`](../../../src/PepperDash.Essentials.Core/Room/IRoomEventSchedule.cs) |
| Language | C# |
| Declaration | `class ScheduledEventEventArgs` with declared base/contract list `EventArgs` |
| Accessibility | `public` |
| Namespace/module | `PepperDash.Essentials.Core` |

## What

`ScheduledEventEventArgs` is an event payload type that carries state-change information between collaborators. This description is grounded in its source declaration and declared inheritance rather than inferred product behavior.
