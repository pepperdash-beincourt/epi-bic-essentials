...
| Source file | [`src/PepperDash.Core.Abstractions/ICrestronEnvironment.cs`](../../../src/PepperDash.Core.Abstractions/ICrestronEnvironment.cs) |
| Language | C# |
| Declaration | `class ProgramStatusEventArgs` with declared base/contract list `EventArgs` |
| Accessibility | `public` |
| Namespace/module | `PepperDash.Core.Abstractions` |

## What

`ProgramStatusEventArgs` is an event payload type that carries state-change information between collaborators. This description is grounded in its source declaration and declared inheritance rather than inferred product behavior.
