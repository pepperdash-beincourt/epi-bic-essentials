...
| Language | C# |
| Declaration | `class DebugConsoleSink` with declared base/contract list `ILogEventSink` |
| Accessibility | `public` |
| Namespace/module | `PepperDash.Core` |

## What

`DebugConsoleSink` is a source-level implementation type with responsibilities defined by its members and inheritance. This description is grounded in its source declaration and declared inheritance rather than inferred product behavior.
...
Preserve the declared inheritance/contract relationship: `ILogEventSink`. Public methods declared in this source file include: `Emit`, `DebugConsoleSink`. Use repository search to identify callers, implementers, serializers, tests, and configuration references before changing a public name or shape.
