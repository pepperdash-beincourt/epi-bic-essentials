...
| Source file | [`src/PepperDash.Core/XSigUtility/Tokens/XSigAnalogToken.cs`](../../../src/PepperDash.Core/XSigUtility/Tokens/XSigAnalogToken.cs) |
| Language | C# |
| Declaration | `class XSigAnalogToken` with declared base/contract list `XSigToken, IFormattable` |
| Accessibility | `public` |
| Namespace/module | `PepperDash.Core.Intersystem.Tokens` |

## What

`XSigAnalogToken` is a source-level implementation type with responsibilities defined by its members and inheritance. This description is grounded in its source declaration and declared inheritance rather than inferred product behavior.
...
Preserve the declared inheritance/contract relationship: `XSigToken, IFormattable`. Public methods declared in this source file include: `GetBytes`, `GetTokenWithOffset`, `ToString`. Use repository search to identify callers, implementers, serializers, tests, and configuration references before changing a public name or shape.
...
Before proposing a change, read this declaration, its full source file, all repository references to `XSigAnalogToken`, and its test coverage. Do not invent configuration keys, payload fields, interface members, or lifecycle ordering. Report the affected source files, tests, and consumer boundaries with any proposed change.
