...
| Language | C# |
| Declaration | `class GenericIrControllerFactory` with declared base/contract list `EssentialsDeviceFactory<GenericIrController>` |
| Accessibility | `public` |
| Namespace/module | `PepperDash.Essentials.Core.Devices` |

## What

`GenericIrControllerFactory` is a construction boundary that maps configuration or discovery inputs into concrete objects. This description is grounded in its source declaration and declared inheritance rather than inferred product behavior.
