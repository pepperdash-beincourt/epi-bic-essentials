...
| Source file | [`src/PepperDash.Core/Comm/GenericTcpIpServer.cs`](../../../src/PepperDash.Core/Comm/GenericTcpIpServer.cs) |
| Language | C# |
| Declaration | `class GenericTcpIpServer` with declared base/contract list `Device` |
| Accessibility | `public` |
| Namespace/module | `PepperDash.Core` |

## What

`GenericTcpIpServer` is a device-model type that owns lifecycle, controls, feedback, or communication responsibilities. This description is grounded in its source declaration and declared inheritance rather than inferred product behavior.
...
Preserve the declared inheritance/contract relationship: `Device`. Public methods declared in this source file include: `ServerHasChokedCallbackDelegate`, `KillServer`, `Initialize`, `Listen`, `StopListening`, `DisconnectClient`, `DisconnectAllClientsForShutdown`, `BroadcastText`, `SendTextToClient`, `GetClientIPAddress`. Use repository search to identify callers, implementers, serializers, tests, and configuration references before changing a public name or shape.
...
Before proposing a change, read this declaration, its full source file, all repository references to `GenericTcpIpServer`, and its test coverage. Do not invent configuration keys, payload fields, interface members, or lifecycle ordering. Report the affected source files, tests, and consumer boundaries with any proposed change.
