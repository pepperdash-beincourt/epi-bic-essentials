...
| Source file | [`src/PepperDash.Essentials.Core/Routing/IRoutingSource.cs`](../../../src/PepperDash.Essentials.Core/Routing/IRoutingSource.cs) |
| Language | C# |
| Declaration | `interface IRoutingSource` with declared base/contract list `IRoutingOutputs, IKeyName` |
| Accessibility | `public` |
| Namespace/module | `PepperDash.Essentials.Core` |

## What

`IRoutingSource` is a contract that callers and implementations use to agree on behavior. This description is grounded in its source declaration and declared inheritance rather than inferred product behavior.
...
Preserve the declared inheritance/contract relationship: `IRoutingOutputs, IKeyName`. Use repository search to identify callers, implementers, serializers, tests, and configuration references before changing a public name or shape.
