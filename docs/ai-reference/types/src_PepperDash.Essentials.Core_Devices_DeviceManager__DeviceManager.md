...
| Source file | [`src/PepperDash.Essentials.Core/Devices/DeviceManager.cs`](../../../src/PepperDash.Essentials.Core/Devices/DeviceManager.cs) |
| Language | C# |
| Declaration | `class DeviceManager` |
| Accessibility | `public` |
| Namespace/module | `PepperDash.Essentials.Core` |

## What

`DeviceManager` is a device-model type that owns lifecycle, controls, feedback, or communication responsibilities. This description is grounded in its source declaration and declared inheritance rather than inferred product behavior.
...
Public methods declared in this source file include: `Initialize`, `ActivateAll`, `DeactivateAll`, `AddDevice`, `RemoveDevice`, `GetDeviceKeys`, `GetDevices`, `GetDeviceForKey`, `SimulateComReceiveOnDevice`, `GetRoutingPorts`, `SetDeviceStreamDebugging`, `DisableAllDeviceStreamDebugging`. Use repository search to identify callers, implementers, serializers, tests, and configuration references before changing a public name or shape.
...
Before proposing a change, read this declaration, its full source file, all repository references to `DeviceManager`, and its test coverage. Do not invent configuration keys, payload fields, interface members, or lifecycle ordering. Report the affected source files, tests, and consumer boundaries with any proposed change.
